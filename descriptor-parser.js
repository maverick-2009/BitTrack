 ...
 * Parse completo de Output Descriptors Bitcoin (BIP380/BIP389)
 *
 * Suporta:
 *   • Chaves com fingerprint+path: [xxxxxxxxx/48'/0'/0'/2']xpub...
 *   • Descritores: pk, pkh, wpkh, sh(wpkh), wsh(multi), wsh(thresh...)
 *   • Miniscript completo dentro de wsh(...)
 *   • Derivação de endereços: xpub.../0/*, xpub.../1/* (change)
 */
...
function b58Decode(str) {
  ...
  for (const c of str) {
    ...
    if (i < 0) throw new Error(`Char inválido base58: '${c}'`);
    n = n * 58n + BigInt(i);
  }
...
function b58CheckDecode(str) {
  ...
  if (!hash.equals(checksum)) throw new Error('Checksum base58check inválido');
  return payload;
}
...
// ─── SECP256K1 (apenas operações necessárias para BIP32) ─────────────────────
// Implementação minimalista — point addition e scalar multiplication
...
// Exponenciação modular — evita BigInt overflow de ** direto
...
function pointMul(k, point = [Gx, Gy]) {
  // Reduz k mod N antes do loop — evita iterações desnecessárias
  k = ((k % N) + N) % N;
...
// Recupera ponto secp256k1 de uma chave pública comprimida
function pubkeyToPoint(pubkeyBuf) {
  ...
  if ((y * y % P) !== y2) throw new Error('Pubkey inválida — ponto fora da curva');
  return [x, y];
}
...
// Version bytes de todas as variantes — normalizados para xpub antes do parse
...
class ExtendedKey {
  constructor({ depth, fingerprint, index, chainCode, key }) {
    ...
    this.key         = key; // Buffer 33 bytes (compressed pubkey)
  }

  static fromBase58(str) {
    const buf = b58CheckDecode(str);
    if (buf.length !== 78) throw new Error(`xpub length inválido: ${buf.length}`);
    // Normaliza ypub/zpub/Ypub/Zpub/tpub/upub/vpub → xpub (mesmos bytes de chave, só version muda)
    const version = buf.slice(0, 4).toString('hex').toLowerCase();
    if (version !== '0488b21e') {
      if (!KNOWN_XPUB_VERSIONS[version]) throw new Error(`Version bytes desconhecidos: ${version}`);
  ...
  // Derivação pública normal (índice < 0x80000000)
  derive(index) {
    if (index >= 0x80000000) throw new Error('Derivação hardened não suportada em xpub');
    ...
    if (ILn >= N) throw new Error('Derivação inválida — IL >= N');

    const parentPoint = this._pubkeyToPoint();
    const childPoint  = pointAdd(pointMul(ILn), parentPoint);
    if (!childPoint) throw new Error('Derivação resultou em ponto no infinito');
  ...
  // Deriva pelo path relativo: "0/0", "0/*" retorna função
  deriveByPath(pathStr) {
    const parts = pathStr.replace(/^m\//, '').split('/').filter(Boolean);
    let node = this;
    for (const part of parts) {
      if (part === '*') return node; // retorna a chave pai para derivação posterior
...
// ─── PARSER DE CHAVE COM ORIGEM ───────────────────────────────────────────────
// Parseia: [fingerprint/path]xpub... ou xpub... ou <hex_pubkey>
function parseKeyExpression(expr) {
  expr = expr.trim();
  let origin = null, keyStr = expr;

  // Extrai origem: [fingerprint/path]
  ...
  // Extrai path de derivação após a chave
  // Suporta: /0/*, /1/*, /<0;1>/*, /* e combinações
  let derivPath = null;
  let multipath = null; // { ext: '0', chg: '1' } para /<0;1>/*
  ...
  if (/^[0-9a-fA-F]{66}$/.test(keyStr)) {
    ...
    catch(e) { throw new Error(`xpub inválido: ${e.message}`); }
  } else if (/^[xyYzZtuUvV]prv/.test(keyStr)) {
    throw new Error('Chaves privadas (xprv) não são aceitas — use apenas xpub');
  } else {
    type = 'named';
  }
...
// ─── DERIVAÇÃO DE PUBKEY ──────────────────────────────────────────────────────
// chain: 0 = externo/recebimento, 1 = change/troco
function derivePublicKey(keyExpr, index = 0, chain = 0) {
  ...
  // BIP389 /<a;b>/*  — escolhe branch pelo chain
  if (multipath) {
...
// ─── PARSER DE DESCRITORES ────────────────────────────────────────────────────
// Tokeniza a expressão respeitando parênteses
function tokenizeDescriptor(str) {
  str = str.trim().replace(/\s+/g, '');
  // Separa checksum (#xxxx) se presente
  const hashIdx = str.lastIndexOf('#');
  if (hashIdx > 0) str = str.slice(0, hashIdx);
  return str;
}

// Parseia um nó do descritor/miniscript recursivamente
function parseNode(str) {
  str = str.trim();

  // Identifica função e argumentos
  const parenIdx = str.indexOf('(');
  if (parenIdx < 0) {
    // É uma expressão de chave folha
  ...
  switch (fn) {
    ...
    case 'tr': {
      // Taproot: tr(internal_key) ou tr(internal_key, script_tree)
...
// Divide argumentos de alto nível respeitando parênteses aninhados
...
// Liftx: recupera ponto com y par da coordenada x (BIP340)
function liftX(xBuf) {
  const x  = BigInt('0x' + xBuf.toString('hex'));
  const y2 = (modPow(x, 3n, P) + 7n) % P;
  let y = modPow(y2, (P + 1n) / 4n, P);
  if ((y * y % P) !== y2) return null;
  // Garante y par (BIP340 usa y par)
  if (y % 2n !== 0n) y = P - y;
  return [x, y];
}
...
function compileTapLeaf(scriptNode, index, chain) {
  ...
  // Tapscript usa x-only keys e regras diferentes — isTapscript=true
...
// Aplica o tweak de Taproot na chave interna (BIP341 key-path)
// Retorna Buffer de 32 bytes (x-only tweaked pubkey)
function taprootTweak(pubkeyBuf, scriptTree = null, index = 0, chain = 0) {
  try {
    const xOnly = pubkeyBuf.slice(1); // 32 bytes, coordenada x

    let merkleRoot;
    if (scriptTree) {
      // Calcula merkle root da script tree
      const leafHash = compileTapLeaf(scriptTree, index, chain);
      // Para tree com um único leaf: merkle root = leaf hash
      // Para tree com múltiplos leaves: H_tapbranch(left, right) — simplificado para 1 leaf
      merkleRoot = leafHash || Buffer.alloc(32);
    } else {
      // Key-path sem script tree: tweak com merkle root vazio
    ...
    // Q = P + t*G  (BIP341)
    // liftX garante que P tem y par; Q pode ter qualquer y, mas usamos só x
    ...
    // Retorna apenas a coordenada x (32 bytes) — BIP340 x-only
...
// Gera o scriptPubKey a partir do nó parseado + índice de derivação
function compileScript(node, index = 0, chain = 0) {
  switch (node.type) {
    case 'tr': {
      ...
      // Key-path sem script tree: tweak = H_taptweak(P)
      const xOnly = pk.slice(1); // remove prefix 02/03, pega só x (32 bytes)
...
// isTapscript: quando true usa x-only keys (32 bytes) e sem OP_DROP em older/after
function compileRedeemScript(node, index = 0, chain = 0, isTapscript = false) {
  switch (node.type) {
    ...
    case 'thresh': {
      // Se todos os filhos são pk/pk_k puros (sem wrapper), usa OP_CHECKMULTISIG clássico
      ...
      // Miniscript thresh — encoding correto:
    ...
    case 'older': {
      ...
      // Miniscript segwit/tapscript: sem OP_DROP — apenas <n> OP_CSV
      // (OP_DROP só era usado em P2SH legado, mas no tipo miniscript V, o CSV
      //  consome a stack via and_v/wrapper v:, não precisa de DROP)
    ...
    case 'wrapped': {
      ...
      for (const w of (node.wrappers || []).reverse()) {
        switch (w) {
          case 'v': {
            // Transforma último opcode em versão VERIFY, ou adiciona OP_VERIFY
          ...
          case 'c': {
            // Adiciona OP_CHECKSIG no final (usado em c:pk_h etc.)
          ...
          // wrappers desconhecidos: ignora silenciosamente
        }
      }
      return { script, type: `WRAP(${child.type})` };
    }

    // ── Combinadores Miniscript ───────────────────────────────────────────────
    case 'and_v': {
      // and_v(X, Y): concatena scripts de X e Y
    ...
    case 'andor': {
      ...
      // andor(X,Y,Z) encoding: X OP_NOTIF Z OP_ELSE Y OP_ENDIF  (Z e Y são trocados!)
...
// Para push de inteiros em scripts: valores 1-16 usam OP_1..OP_16 (0x51..0x60)
// valores 0 usam OP_0 (0x00), resto usa pushData(encodeScriptNum)
...
function analyzeDescriptor(descriptorStr, opts = {}) {
  ...
  // Deriva endereços — aceita { index, chain } ou só index (compatibilidade)
...
function addressToScript(address) {
  ...
  throw new Error('Endereço não suportado');
}
...
function detectScriptType(node) {
  switch(node.type) {
    ...
    default:      return node.type?.toUpperCase() || 'Desconhecido';
  }
}
...
// ─── CLI (node descriptor-parser.js "<descritor>" [opções]) ──────────────────
// Opções:
//   --range <inicio> <fim>   deriva índices de <inicio> até <fim> inclusive  (padrão: 0 0)
//   --index <n> [n2 ...]     deriva índices específicos
//   --change                 usa chain 1 (change) em vez de chain 0 (recebimento)
//
// Exemplos:
...
if (require.main === module) {
  ...
  if (rangeIdx >= 0) {
    ...
    if (isNaN(start) || isNaN(end)) {
      console.error('❌ --range requer dois números: --range <inicio> <fim>');
      process.exit(1);
    }
    deriveSpec = [];
  ...
  if (indexIdx >= 0) {
    ...
    if (!deriveSpec.length) {
      console.error('❌ --index requer ao menos um número: --index 0 1 2');
      process.exit(1);
    }
  ...
  if (!descriptor) {
    console.log(`
Uso: node descriptor-parser.js "<descritor>" [opções]

Opções:
  --range <inicio> <fim>    deriva índices de inicio até fim  (ex: --range 0 9)
  --index <n> [n2 ...]      deriva índices específicos        (ex: --index 0 1 5)
  --change                  usa chain 1 (change/troco)
...
`);
  ...
  // Padrão: índice 0 se nenhuma opção foi passada
  ...
  try {
    ...
    console.log('\n' + '═'.repeat(60));
    console.log(' ANÁLISE DO DESCRITOR');
    console.log('═'.repeat(60));
    console.log(`\n Tipo:       ${result.scriptType}`);
    console.log(` Miniscript: ${result.miniscript ? 'Sim ✅' : 'Não'}`);
    ...
    result.keys.forEach((k, i) => {
      ...
      if (k.derivPath) console.log(`   Derivação:   /${k.derivPath}`);
    });
    ...
    if (result.addresses.length) {
      ...
      console.log('\n' + '─'.repeat(60));
      console.log(` ENDEREÇOS DERIVADOS — ${chainLabel} — índices: ${idxLabel}`);
      console.log('─'.repeat(60));
